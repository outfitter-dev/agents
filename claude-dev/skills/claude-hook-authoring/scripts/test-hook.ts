...
async function main() {
  ...
  console.log(`${colors.blue}Testing hook script: ${scriptPath}${colors.reset}`);
  console.log();
  ...
  try {
    result = await runHook(resolvedPath, input, options.timeout, options.verbose);
  } catch (error) {
  ...
  if (exitCode === 0) {
    console.log(
      `${colors.green}✓ Hook executed successfully${colors.reset}`
    );
    if (result.stdout) {
